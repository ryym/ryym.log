...
- 免責事項
- 自分にとってのテスト
  - 概念的にはどんな処理も「引数->処理->結果」
  - 内部実装に依存せず「この引数ならこの結果になる」ことを確かめる行為
  - (モックは結果よりも過程の検証に使われるが) テストダブルを使うにしても、あくまでテスト時に実行できない関連処理を代替する目的 (stub) がメイン
  - 外部サービスにデータを登録するなど、結果が検証不可能な場合のみ、過程を検証することがゴールになる
  - -> なのでテストダブルは使わずに済むならその方が良いと思う
- テストダブルを局所的に使いたい場合の課題: 間接的な依存の差し替え
  - 直接の依存を全て常に差し替える方針なら、この問題は発生しない
  - 厄介な副作用のみを差し替える方針なら、どんな方法でもトレードオフが発生する
- 1つの案として、アプリケーションがテストのユースケースまでカバーするのはどうか
  - 引数によって実際の処理ではなくシミュレートバージョンの挙動にできる
  - アプリがテストを意識するのは普通避けるべきだが、どうせテストのためにインターフェイス作ったりするくらいなら
  - (ここで何段階かの依存チェーンが、間接的な依存の詳細を知る必要なく、自身の直接の依存に対してだけ振る舞いを指定するいい感じの例が非常に必要)
  - メリット
    - あるクラスに関する処理が、テストダブルというかシミュレート方法を含めて一箇所にまとまる
    - 各処理にどんなパターンがあるのか、シミュレートのインターフェイスで可視化できる
    - 引数を外部から渡せれば、単体テストのみならず結合テストや e2e (?) テストでも使える
      - テストのみならず、滅多に起きないケースをデモしたりデバッグしたり
      - 関連マイクロサービスも同様の test-aware なインターフェイスを持てば、サービス間の結合までしつつシミュレートもできる
  - デメリット
    - 実装が複雑になるかも
      - そもそも引数で色んなパターンを指定するのが現実的なのかどうか
    - アプリケーションコードに意図せぬ影響を与える可能性が0ではない
- 過程の検証が必要なテストをどうするか
  - 何かしらの方法で過程の検証を結果の検証に置き換えられるとよい
  - 戻り値として表現できるならそれがベスト
    - 単なる戻り値だと「戻り値だけ返して実際の処理が走ってない」もありえるので、実際に処理を走らせないと返せない値を含める
  - 戻り値が難しければ、 DB のテーブルでもグローバル変数でも、適当な場所に値を書き込む、という stub にすればいい
    - こうすれば処理と caller の間にはその副作用の内容に関する合意のみが必要で、内部でどんな API クライアントをどんなふうに呼ぶか、という実装詳細は隠蔽できる
    - そうなれば過程の検証ではなく、 stub ベースの test-aware 方針に乗れる

---

- この案で主に念頭にあるのは stub だな。 behavior を検証するための mock ではない。
  そういう、直接的に処理の戻り値には表現されない副作用をどう検証するか、はまた別のトピック感。
