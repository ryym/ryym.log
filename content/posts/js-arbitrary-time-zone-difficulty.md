...
特に特定のタイムゾーンで日時処理したい時や、複数のタイムゾーンを扱いたい場合に困る。
...
この問題は DST へ切り替わる日時において発生する。逆に言うとそれ以外の日時では (たぶん) 期待通りに動くので、 `withUTCOffset` 相当の実装でも問題ないユースケースはありそう。どこまで厳密さを求めるかによる。
...
あくまでローカルのタイムゾーンが不定という前提なら、 `Date` を UTC 日時ベースでのみ使いつつオフセットを時前で管理するか。DST があると難しそうだけど。  

ただそもそも単純な日時の表示や入力のみであれば、UI としてはユーザ環境のタイムゾーンに則るのがむしろ自然なので、実は気にしなくて良いケースも多いはず。保存形式に注意するくらい。考慮が必要になるのは、例えば入力された日時を特定のタイムゾーンにおける上限・下限で範囲チェックしたい時とか。
