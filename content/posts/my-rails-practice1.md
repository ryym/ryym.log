...
draft = false
tags = ["ruby", "rails"]
title = "Rails 開発で意識していること 1 - 方針編"
updated = "2019-07-09T10:18:03+09:00"
+++

Rails での開発を始めて結構経つので、
ふと思いたって普段の開発時に考えている諸々を文章化してみる事にしました。
...
- Rails 開発で意識していること 1 - 方針編 (この記事)
- [Rails 開発で意識していること 2 - 詳細編][my-rails-practice-2]

[my-rails-practice-2]: /posts/my-rails-practice2/

## はじめに

[clean-architecture-book]: www.amazon.co.jp/dp/B07FSBHS2V
...
プロジェクトのフェーズや性質にもよるでしょうが、一般にソフトウェアは求められる要件や周囲の状況に応じて柔軟に変わり続ける (べき) ものであり、それが可能である事こそソフトウェアの「ソフト」ウェアたる意義であるはずです。  
...
よってアプリのビジネスロジックは、なるべくフレームワークから隔離して管理したいです。
そこで一般的には`app/services`のような独自レイヤーを設ける事が多いと思います。
自分もよくそうしますが、詳細は後述します。

## ミックスインは極力使わない

次に個人的に徹底したいのはこれです。
Ruby に限らず、動的型付け言語でのミックスイン機構ってあまり好きではありません。
...
これは特に複数のモジュールを include すると顕著で、どのメソッドがどのミックスインに属しているのかを形式的に判断する方法がなく、せいぜいモジュール名とメソッド名から類推するしかありません。
...
`after_create`というクラスメソッドや`id`、`name`というカラムを使っています。
モデルに include される前提なら`after_create`や`id`はまだ大丈夫そうですが、`name`という特に一般的ではないカラムにまで依存しています。
...
更にはテスタビリティの問題もあります。
ミックスインという形でロジックを切り出すと、ミックスイン単体でのテストも、インクルーダ側のテストもしづらくなると感じます。
...
というわけで、 Ruby でコードをモジュール化するなら基本はプレーンなクラスとして実装するのが簡単で良いと思います。
...
- モジュールはクラスとして表現する。
- クラスはコンストラクタで依存を明示的に受け取る。
- モジュールを使う側は、そのクラスが必要とする依存を知らずに済むようにする。

という感じです。コードにするとこんな感じのイメージになります:
...
また基本モジュールはステートレスになるでしょうから、`create`ではなく`instance`などとしてシングルトンで運用しても良いかもしれません。

### Rails ではどうなる？

とはいえ、素の Rails にはプレーンなクラスなんてほとんど登場しません。
Rails の規約に沿って作るクラスはコントローラなりモデルなり、フレームワークのクラスを継承して始めから何かしらの責務を担っています。
しかし前述のようにビジネスロジックをフレームワークから分離するなら、そこではこの手の方針を適用する事ができます。

---

続き - [Rails 開発で意識していること 2 - 詳細編][my-rails-practice-2]
